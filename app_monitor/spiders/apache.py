...
class ApacheSpider(scrapy.Spider):
    ...
    repos = [
        {
            "repo": "apache-karaf-runtime",
            "name": "Apache Karaf Runtime",
            "url": "https://dlcdn.apache.org/karaf/",
            "tags": ["java", "osgi", "karaf", "apache"],
        },
        {
            "repo": "apache-karaf-cave",
            "name": "Apache Karaf Cave",
            "url": "https://dlcdn.apache.org/karaf/cave/",
            "tags": ["java", "osgi", "karaf", "apache"],
        },
        {
            "repo": "apache-karaf-cellar",
            "name": "Apache Karaf Cellar",
            "url": "https://dlcdn.apache.org/karaf/cellar/",
            "tags": ["java", "osgi", "karaf", "apache"],
        },
        {
            "repo": "apache-karaf-decanter",
            "name": "Apache Karaf Decanter",
        ...
        {
            "repo": "apache-tomcat9",
            "name": "Apache Tomcat 9.x",
            "url": "https://dlcdn.apache.org/tomcat/tomcat-9/",
            "tags": ["java", "tomcat", "web", "apache"],
        },
        {
            "repo": "apache-tomcat10",
            "name": "Apache Tomcat 10.x",
    ...
    def _get_latest_version(self, links):
        ...
        versions = list(
            map(
                lambda x: dict(original_ver=x, ver=parse(x)),
    ...
    def _parse_latest_version_contents(self, response, version, id, name, tags):
        if len(response.xpath('//a[starts-with(text(), "binaries")]').getall()) > 0:
            yield scrapy.Request(
                response.url + "binaries",
                callback=self._parse_latest_version_contents,
                cb_kwargs=dict(version=version, id=id, name=name, tags=tags),
            )
        elif len(response.xpath('//a[starts-with(text(), "bin")]').getall()) > 0:
            yield scrapy.Request(
                response.url + "bin",
                callback=self._parse_latest_version_contents,
                cb_kwargs=dict(version=version, id=id, name=name, tags=tags),
    ...
    def _parse_repo(self, response, **kwargs):
        core_version = self._get_latest_version(
            response.xpath("//pre//a/text()").getall()
        )
        ...
        yield scrapy.Request(
            ...
            callback=self._parse_latest_version_contents,
            cb_kwargs=dict(
                version=core_version["original_ver"],
                id=kwargs["repo"],
                name=kwargs["name"],
                tags=kwargs["tags"],
            ),
        ...
        if hasattr(self, "repo") and len(self.repo) > 0:
            ...
            for r in self.repos:
                if r["repo"] == self.repo:
                    ...
                    yield Request(r["url"], cb_kwargs=r)
                    hit = True
                    break
            if not hit:
            ...
            for repo in self.repos:
                ...
                yield Request(repo["url"], cb_kwargs=repo)
